...
<head>
    ...
    <link rel="stylesheet" href="css/page-aboutRent.css" type="text/css">
            ...
            for(let i=0;i<all.length;i++){
                if(all[i].innerHTML === Xpath){
                    ...
                    for(let j=0;j<allC.length;j++){
                        if(Xpath === allC[j].getElementsByTagName("h1")[0].innerText){
...
<div class="content w clearfix">
    <div class="contents">
        ...
        <s:iterator value="resultInfo.data.list" var="name" status="st">
            <div><a href="javascript:void(0);" class="href"><s:property value="#name.item"/></a></div><hr>
        </s:iterator>
        </div>
        <s:iterator value="resultInfo.data.list" var="name" status="st">
            <div class="details" style="display: none;">
                <s:property value="#name.content" escapeHtml="false"/>
                <a href="javascript:void(0);" class="return">返回资讯列表</a>
            </div>
        </s:iterator>
        <div class="pageNum">
            <ul>
